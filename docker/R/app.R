...
frow2<-fluidRow(box(    title = "Rooted Phylogenetic Tree Plot"    ,status = "primary"   ,solidHeader = TRUE    ,collapsible = TRUE     ,plotOutput("besttree", height = "300px") ),box(    title = "Model Test Results"    ,status = "primary"   ,solidHeader = TRUE    ,collapsible = TRUE     ,textOutput("test") ))
frow3<-fluidRow(valueBoxOutput("aic"))
header <- dashboardHeader(titleWidth = 1250,title = "RpNRM: Rooting Phylogenetic Trees using a Non-Reversible Nucleotide Substitution Model")
...
sidebar <- dashboardSidebar(menuItem("OWNERSHIP", tabName = "model",
                            ...
                            menuItem("ABOUT",tabName="model",box(title="Application Information",background = "green",width = 150,
                                                                 "RpNRM is a web application that roots phylogenetic trees using a 12-rate non-reversible nucleotide substitution stocastic model (NREV12). For any uploaded phylogenetic tree, the application will firstly conduct a model test using hyphy (hypothesis testing using phylogenies) to compare the goodness of fit of GTR (6-rate reversible model) and the NREV12 model. If NREV12 is the best suited model for the dataset, RpNRM will then root the phylogenetic tree on the most porbable branch. If NREV12 is not the best suited model for your dataset, we advise you root the tree using other softwares.")),
                            ...
                            menuItem("INSTRUCTIONS",tabName = "model",box(title="Instructions",
                                                                          ...
                                                                          "Upload a phylogenetic tree in newick format and a sequence alignment in fasta format. Once the upload is complete, Run the application by clicking on the run button.")),menuItem("DATA INPUT", tabName = "model",
                                                                                                                                                                                                                                                             fileInput("file1", "Choose newick file", accept = c(".nwk",".tree",".tre"), multiple = FALSE),fileInput("file2", "Choose a fasta file", accept = c(".fasta",".fas"), multiple = FALSE),actionButton("Run",label = "Run"),
...
server <- function(input, output,session) {
  ...
  read_newick_tree_and_run <- eventReactive(input$Run,{
    ...
    file.copy(input$file1$datapath, "/srv/shiny-server/rpnrm", overwrite = TRUE)
    tree_load<-read_file()
    tree<-read.tree(tree_load)
    reroot.all(tree)
    req(input$file2$datapath)
    ##copy from r datapath to direc
    file.copy(input$file2$datapath, "/srv/shiny-server/rpnrm", overwrite = TRUE)
    ...
    if(v[3,] > v[1,] || v[3,] > v[2,]){
      ...
      output$test<-renderText({"MODEL TEST RESULTS: NREV12 is not the best fitting model for your dataset. Please consider rooting the tree with another software."})
    } else{
      ...
      output$test<-renderText({"MODEL TEST RESULTS: NREV12 is the best fitting model for your dataset."})
  ...
  output$downloadData<-downloadHandler(
    ...
    content = function(RootedTree){
      file.copy("/srv/shiny-server/rpnrm/rooted.nwk", RootedTree, overwrite = TRUE)
    },
